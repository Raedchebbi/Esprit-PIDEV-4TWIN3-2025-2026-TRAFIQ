...
const CONGESTION_LEVELS = {
  Fluide: { icon: '🟢', color: '#2E7D32', bg: '#E8F5E9' },
  Modéré: { icon: '🟡', color: '#F57C00', bg: '#FFF3E0' },
  Dense: { icon: '🟠', color: '#E65100', bg: '#FBE9E7' },
  Saturé: { icon: '🔴', color: '#B71C1C', bg: '#FFEBEE' },
};
...
export default function RouteRecommendationCard({ route, selected, onSelect }) {
  ...
  const congestion = CONGESTION_LEVELS[congestionLevel] || CONGESTION_LEVELS.Fluide;
    ...
    >
      ...
      <div className="rec-card__stats">
        <div className="rec-card__stat">
          <span className="rec-card__stat-icon">⏱</span>
      ...
      {incidentCount > 0 ? (
        <div className="rec-card__incidents rec-card__incidents--active">
          🚨 {incidentCount} incident(s) actif(s) sur ce trajet
        </div>
      ) : (
